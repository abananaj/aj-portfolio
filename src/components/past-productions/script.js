document.addEventListener("DOMContentLoaded", function () {
  ...
  var background = document.querySelector(".bg-mouse-track");
  ...
  cards.forEach(function (card, index) {
    ...
    card.addEventListener("click", function () {
      if (card.classList.contains("zoomed")) {
        ...
        // bg opacity to 1 when a card is unzoomed
        background.style.opacity = "1";

        // overflow visible when a card is unzoomed
        document.body.style.overflow = "visible";

        // change other card's opacity to 0 when a card is unzoomed
        cards.forEach(function (otherCard) {
          if (otherCard !== card) {
            otherCard.style.opacity = "100%";
          }
        });
      } else {
        ...
        // change other card's opacity to 0 when a card is zoomed
        document.body.style.overflow = "hidden";

        // change other cards opacity when a card is zoomed
        cards.forEach(function (otherCard) {
          if (otherCard !== card) {
